...
def database_tags()
    ...
    return db.execute("SELECT *, tag.name as name, tag_purpose.name as tag_purpose_name FROM tag
...
def database_create_tag(tag_name, tag_purpose_id)
    ...
    db.execute("INSERT INTO tags (name, tag_purpose_id) VALUES (?, ?)", tag_name, tag_purpose_id)
end
