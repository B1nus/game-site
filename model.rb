...
module Model
  ...
  def database
    connect_to_database('data/db.db')
  end
  ...
  def database_game_iframe_size(game_id)
    database.execute(
      ...
      tag_purpose_id = tag_purpose.id WHERE game_id = ? AND tag_purpose.name = \'iframe_size\'',
      game_id
    ).map { |e| e['tag_name'] }
  end
  ...
  def database_game_available_tags(game_id)
    database.execute(
      ...
      tag_purpose_id WHERE gtr.game_id IS NULL;',
      game_id
    )
  end
  ...
  def database_game_tag_purposes(game_id)
    database.execute(
      ...
      ON tag.tag_purpose_id = tag_purpose.id WHERE game_id = ?',
      game_id
    ).map { |e| e['name'] }
  end
